...
- ELECTIVE-COURSE-001 ~ 003：实验课程开放选课、重复场次、任务按实验课程发放、师生课表联动、实验课程签到、手动报名/禁选名单。
- LAB-ROOM-OPENING-001：实验室信息升级为结构化开放时段；教师端与管理员端共用全局实验室库。
...
- `lab_room`
  - 实验室信息（名称唯一；地点/开放时间摘要）
- `lab_room_open_slot`
  - 实验室按周开放时段（`weekday` + `start_time` + `end_time`）
  - 外键：`lab_room_id -> lab_room.id`
- `device`
  - 设备台账（编码唯一；状态/位置/描述）
- `semester`
  - 学期设置（名称唯一；起止日期可选）
...
### 5.6 教师端实验课程与实验室（/api/teacher/**）
- 实验课程
  - `GET/POST/PUT /api/teacher/experiment-courses`
  - `PUT /api/teacher/experiment-courses/{courseId}/status`
  - `GET /api/teacher/experiment-courses/{courseId}/enrollments`
  - `GET /api/teacher/experiment-courses/{courseId}/roster`
  - `POST /api/teacher/experiment-courses/{courseId}/roster/enroll`
  - `POST /api/teacher/experiment-courses/{courseId}/roster/remove`
  - `DELETE /api/teacher/experiment-courses/{courseId}/blocked-students/{studentId}`
- 实验室管理（教师与管理员共用同一套全局实验室库）
  - `GET/POST/PUT/DELETE /api/teacher/lab-rooms`
  - 请求/返回与管理员端实验室接口一致
  - `openSlots` 采用结构化周规则：`weekday`、`startTime`、`endTime`
